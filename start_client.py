...
if __name__ == "__main__":
    ...
    if not all([daemon_dir.is_dir(), cli_project_dir.is_dir(), (cli_project_dir / "pyproject.toml").is_file()]):
        print("[FATAL] Project structure incorrect. Ensure 'pyproject.toml' is in the root. Halting.")
        sys.exit(1)
    ...
    run_command(["pip", "install", "--upgrade", "pip"], cwd=daemon_dir, venv_python_exe=python_exe)
    run_command(["pip", "install", "PyQt6", "qtawesome", "watchdog"], cwd=daemon_dir, venv_python_exe=python_exe)
    
    # --- 核心修复：使用正确的路径进行安装 ---
    run_command(["pip", "install", "-e", str(cli_project_dir)], cwd=daemon_dir, venv_python_exe=python_exe)
